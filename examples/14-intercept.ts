 ...
 *
 * Background: a still frame (only the target moves) or the video itself, looped — then B may just as
 * well capture something that moves in the video: it only knows "small and moving".
 */
...
await startLiveServer({
  ...
  root: SAMPLES_DIR,
  controlsHtml: `
    <label>Background ${select('bg', [['still', 'Still frame — only the ball moves'], ['video', 'The video, looped — with distractors']], 'still')}</label>
  ...
  frames: (source, params, size) => {
    ...
    return {
      input: params.get('bg') === 'video' ? 'loop' : 'still',
